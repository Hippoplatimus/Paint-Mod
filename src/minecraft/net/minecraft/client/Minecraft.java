// Modified to allow paintbrushes to work correctly
    ...
    implements Runnable
{
    ...
    public static RenderEngine hack;
    ...
    private void clickMouse(int i)
    {
        ...
        if(objectMouseOver.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            } else
            {
                ...
                PaintWorld.instance.possiblePaint(objectMouseOver);
                boolean placed;
                if (ItemPaintbrush.hasPaint(itemstack2.getItem())) {
                	placed = itemstack.useItem(thePlayer, theWorld, i, j, k, i1);
                }
                else {
                	placed = playerController.sendPlaceBlock(thePlayer, theWorld, itemstack2, j, k, l, i1);
                }
                if(placed)
                {
                    flag = false;
                    if (PaintWorld.instance.paintClickEnd()) {
                    	rightClickDelayTimer = 0;
                    	PaintWorld.instance.resetPaintCommitted();
					}
                    else
                    	thePlayer.swingItem();
                }
    ...
    public void runTick()
    {
        ...
        if (ticksRan % 30 == 0) {
        	PaintWorld.instance.flushSendQueue(getSendQueue());
        }
